...
export const TableMenuFeature = () => {
    const { t } = useTranslation("tableMenu");
            ...
            >
                <div className="relative h-32 flex-shrink-0 bg-slate-900">
                    ...
                    <div className="absolute bottom-3 left-4 flex items-center gap-2">
                        ...
                        <p className="text-sm font-bold text-white">
                            {table ? table.name : tableId && !isTableLoading ? t("notFound.title") : t("menu.defaultTitle")}
                        </p>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto pb-6">
                    <div className="px-4 pt-4 space-y-3">
                        {isLoading ? (
                            <p className="text-xs text-slate-400 text-center py-8">{t("menu.loading")}</p>
                        ) : filteredItems.length === 0 ? (
                            <p className="text-xs text-slate-400 text-center py-8">{t("menu.noItems")}</p>
                        ) : (
