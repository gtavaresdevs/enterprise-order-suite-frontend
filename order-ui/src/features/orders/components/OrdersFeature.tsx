        ...
        <div className="min-h-full" style={{ fontFamily: "'Outfit', sans-serif" }}>
            ...
            <div className="relative z-10 max-w-[1200px] mx-auto px-6 py-8">
                <div className="flex items-start justify-between mb-8">
                    <div>
                        <div className="flex items-center gap-2 mb-1"><ShoppingCart className="w-4 h-4 text-slate-400" /><span className="text-xs font-medium text-slate-400 uppercase tracking-widest">{t("feature.brandLabel")}</span></div>
                        <h1 className="text-2xl font-semibold text-slate-900">{t("feature.title")}</h1>
                        <p className="text-sm text-slate-400 mt-1">{t("feature.subtitle")}</p>
                    </div>
                    <button onClick={() => setCreateOpen(true)} className="inline-flex items-center gap-2 h-9 px-4 rounded-[8px] bg-slate-950 text-slate-50 text-sm font-semibold border border-slate-800 shadow-inner hover:bg-slate-800 active:scale-[0.98] transition-all mt-1">
                        <Plus className="w-3.5 h-3.5" /> {t("feature.newOrderButton")}
                    </button>
                </div>
                    ...
                    {(["New", "Preparing", "Ready", "Completed", "Cancelled"] as OrderStatus[]).map((s) => {
                        ...
                        return (
                            <div key={s} className="bg-white rounded-[8px] border border-slate-100 px-4 py-3.5 flex items-center justify-between">
                                <div>
                                    <p className="text-xs text-slate-400 font-medium mb-1 uppercase tracking-wide">{statusLabel(s)}</p>
                ...
                <div className="sticky top-4 z-30 mb-4">
                    <div className="bg-white/90 backdrop-blur-md border border-slate-100 rounded-[8px] px-4 py-3 flex items-center gap-3 shadow-sm">
                        <div className="relative flex-1">
                            ...
                            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("feature.searchPlaceholder")} className="w-full h-9 pl-9 pr-4 rounded-[8px] bg-slate-50 border border-slate-200 text-sm text-slate-800 placeholder-slate-400 outline-none focus:ring-2 focus:ring-slate-950/10 focus:border-slate-300 transition-all" />
                        </div>
                        ...
                        <div className="flex items-center gap-1.5">
                            {FILTERS.map((f) => (
                                <button key={f} onClick={() => setActiveFilter(f)} className={`h-7 px-3 rounded-[8px] text-xs font-medium transition-all ${activeFilter === f ? "bg-slate-950 text-slate-50 border border-slate-800 shadow-inner" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}>
                                    {statusLabel(f)}<span className="ml-1.5 font-mono text-slate-400">{counts[f] ?? 0}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-[8px] border border-slate-100 shadow-sm overflow-hidden">
                    <div className="grid grid-cols-[1fr_1.2fr_0.8fr_0.7fr_0.8fr_0.7fr_auto] gap-4 items-center px-5 py-3 border-b border-slate-100 bg-slate-50/70">
                        {[t("table.colOrderId"), t("table.colCustomer"), t("table.colItems"), t("table.colDate"), t("table.colTotal"), t("table.colStatus"), ""].map((col, i) => (
                            <span key={i} className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{col}</span>
                        ))}
                    </div>
                    {isLoading ? (
                        <div className="py-16 flex justify-center">
                            <p className="text-sm text-slate-400 font-mono animate-pulse">{t("table.loading")}</p>
                        </div>
                    ) : filteredOrders.length === 0 ? (
                        <div className="py-16 flex flex-col items-center gap-3">
                            <Package className="w-8 h-8 text-slate-200" />
                            <div className="text-center"><p className="text-sm font-medium text-slate-500">{t("table.emptyTitle")}</p><p className="text-xs text-slate-400 mt-0.5">{t("table.emptySubtitle")}</p></div>
                        </div>
                    ) : (
                    ...
                    <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
                        <span className="text-xs text-slate-400 font-mono">{t("table.showingCount", { shown: filteredOrders.length, total: orders.length })}</span>
